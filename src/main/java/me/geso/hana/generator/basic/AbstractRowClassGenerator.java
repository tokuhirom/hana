...
public class AbstractRowClassGenerator extends Renderer {
	...
	private void renderHasManySearchChildren(ExportedKey e) {
		appendf("	public List<%s> %s() throws SQLException, HanaException {\n",
				...
				configuration.generateSearchChildrenMethodName(e
						.getForeignKeyTable()));
		appendf("		return this.currentSession().search(%s.class).where(me.geso.hana.Condition.eq(\"%s\", this.%s)).getResultList();\n",
				configuration.generateConcreteFullClassName(e
						.getForeignKeyTable()),
	...
	private void renderHasManyCount(ExportedKey e) {
		...
		appendf("		return this.currentSession().search(%s.class).where(me.geso.hana.Condition.eq(\"%s\", this.%s)).count();\n",
	...
	private void renderHasARetrieve(ImportedKey e) {
		...
		appendf("		return this.currentSession().search(%s.class).where(me.geso.hana.Condition.eq(\"%s\", this.%s)).first();\n",
				configuration.generateConcreteFullClassName(e
						.getPrimaryKeyTable()),
				e.getPrimaryKeyColumn(),
				e.getForeignKeyColumn());
